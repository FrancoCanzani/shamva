...
async function sendHeartbeat() {
  try {
    await fetch('https://your-domain.com/api/heartbeat?id=YOUR_HEARTBEAT_ID&service=YourServiceName');
...
async function sendHeartbeat() {
  try {
    const response = await fetch('https://your-domain.com/api/heartbeat?id=YOUR_HEARTBEAT_ID');
    if (!response.ok) {
