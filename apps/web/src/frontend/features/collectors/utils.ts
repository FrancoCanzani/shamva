...
export function calculatePrevPeriodAverage(metrics: Metric[], key: keyof Metric): number {
...
export function getCurrentPeriodAverage(metrics: Metric[], key: keyof Metric): number {
  const sortedMetrics = [...metrics].sort((a, b) => 
    new Date(a.created_at).getTime() - new Date(b.created_at).getTime()
  );
